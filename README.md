...
favicon.thinking() / .loading() / .processing() / .syncing() /
  .success() / .error() / .warning() / .offline() / .notification(): this
...
Open [`demo/index.html`](./demo/index.html) after running `npm run build` —
click "Start task," then switch to another tab and watch this one instead.
...
If `live-favicon` saves you time, consider [buying the maintainer a coffee](https://buymeacoffee.com) or sponsoring on GitHub Sponsors. ☕
